...
const Temarios: React.FC = () => {
  ...
  const handleCreateTemario = () => {
    handleEditTemario(null, handleSaveTemario); // Pasar null para indicar que es un nuevo temario y pasar la función de guardar
  };

  // Función para guardar un temario (nuevo o editado)
  const handleSaveTemario = (temario: Temario) => {
    console.log('BaseTemarios - handleSaveTemario - Temario recibido:', temario);
    // Verificar si es un nuevo temario o uno existente
    const isNew = !temarios.some(t => t.id === temario.id);
    
    if (isNew) {
      console.log('BaseTemarios - handleSaveTemario - Agregando nuevo temario');
      // Si es nuevo, agregarlo a la lista
      setTemarios([...temarios, temario]);
    } else {
      console.log('BaseTemarios - handleSaveTemario - Actualizando temario existente');
  ...
  const handleConfirmDelete = (id: number) => {
    console.log('BaseTemarios - handleConfirmDelete - ID recibido:', id);
    ...
    <div className="bg-light min-vh-100">
      <Container className="py-5">
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <Button variant="danger" onClick={handleCreateTemario}>
                ...
                <Form.Group>
                  ...
                  >
                    ...
                    {GRADOS_CATALOGO.map((grado, index) => (
                      <option key={index} value={grado}>Grado {grado}</option>
                    ))}
                  </Form.Select>
                </Form.Group>
              </Col>
                          ...
                          <td>
                            <div className="d-flex justify-content-center gap-2">
                              ...
                              <Button 
                                ...
                                size="sm"
                                style={{width: '40px'}}
                                onClick={() => handleDeleteTemario(temario, handleConfirmDelete)}
